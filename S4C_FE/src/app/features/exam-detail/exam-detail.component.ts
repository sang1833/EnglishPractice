      ...
      <div class="max-w-[1000px] mx-auto px-4">
        ...
        @if (errorMessage()) {
          <div class="text-center py-16 px-8 bg-surface rounded-lg">
            ...
            <button class="px-6 py-3 rounded-md border-none font-medium cursor-pointer transition-all bg-[image:var(--gradient-primary)] text-text-inverse" (click)="loadExam()">{{ 'COMMON.TRY_AGAIN' | translate }}</button>
          </div>
        }
        ...
        @if (exam()) {
          <article>
              ...
              <div class="p-8 flex-1">
                ...
                <div class="flex flex-wrap gap-6">
                  ...
                  <span class="flex items-center gap-2 text-text font-medium">
                    ...
                    {{ getTotalSections() }} {{ 'EXAM.SECTIONS' | translate }}
                  </span>
                </div>
              </div>
            ...
            <section class="bg-surface rounded-xl p-8 mb-8 shadow-[var(--shadow-card)]">
              <h2 class="text-xl font-semibold text-text mb-6">{{ 'EXAM.TEST_STRUCTURE' | translate }}</h2>
              <div class="grid grid-cols-[repeat(auto-fit,minmax(200px,1fr))] gap-4">
                @for (skill of skills(); track skill.id) {
                  <div class="flex items-center gap-4 p-4 bg-surface-alt rounded-lg transition-transform hover:translate-x-1">
                    ...
                    <div>
                      <h3 class="text-base font-semibold text-text mb-1">{{ getSkillKey(skill.skill) | translate }}</h3>
                      <p class="text-sm text-muted m-0">
                        {{ skill.sectionCount }} {{ 'EXAM.PARTS' | translate }} • {{ skill.duration }} {{ 'COMMON.MINS' | translate }}
                      </p>
                    </div>
                  </div>
                }
            ...
            <!-- Mode Selection -->
            <section class="my-8">
                <h2 class="text-xl font-semibold text-text mb-6">{{ 'EXAM.TEST_MODE' | translate }}</h2>
                <div class="flex gap-4 flex-wrap">
                    ...
                    >
                        ...
                        <span class="flex flex-col">
                            <span class="font-semibold text-text">{{ 'EXAM.FULL_TEST' | translate }}</span>
                            <span class="text-sm text-muted">{{ 'EXAM.FULL_TEST_DESC' | translate:{duration: exam()!.duration} }}</span>
                        </span>
                    </label>
                    <label 
                      class="flex-1 min-w-[200px] flex items-center gap-3 p-4 border-2 border-border rounded-lg cursor-pointer transition-all bg-surface hover:border-border-hover"
                    ...
                    >
                        ...
                        <span class="flex flex-col">
                            <span class="font-semibold text-text">{{ 'EXAM.PRACTICE_MODE' | translate }}</span>
                            <span class="text-sm text-muted">{{ 'EXAM.PRACTICE_MODE_DESC' | translate }}</span>
                        </span>
                    </label>
                </div>
            </section>
            ...
            @if (isPracticeMode()) {
                <section class="bg-surface p-6 rounded-lg mb-8 border border-border">
                    <h3 class="text-base font-semibold text-text mb-4">{{ 'EXAM.SELECT_SKILLS' | translate }}</h3>
                    <div class="flex gap-6 flex-wrap">
                        @for (skill of availableSkills(); track skill) {
                            ...
                            >
                                ...
                                />
                                <span class="font-medium text-text">{{ getSkillKeyByName(skill) | translate }}</span>
                            </label>
                        }
                    </div>
                </section>
            ...
            <section class="flex flex-col md:flex-row gap-4">
              ...
              >
                @if (isStarting()) {
                  ...
                  {{ 'EXAM.STARTING' | translate }}
                } @else {
                   {{ (isPracticeMode() ? 'EXAM.START_PRACTICE' : 'EXAM.START_FULL') | translate }}
                }
              </button>
              <button 
      ...
      @if (showResumeModal()) {
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-[1000] p-4" (click)="cancelResume()">
          <div class="bg-surface rounded-xl p-8 max-w-[400px] w-full text-center" (click)="$event.stopPropagation()">
            <h2 class="text-xl font-bold text-text mb-4">{{ 'EXAM.RESUME_TITLE' | translate }}</h2>
            <p class="text-muted mb-2">{{ 'EXAM.RESUME_MSG' | translate }}</p>
            <p class="font-medium text-text mb-6">{{ 'EXAM.STARTED' | translate:{date: pendingAttempt()?.startedAt | date:'medium'} }}</p>
            <div class="flex gap-4">
              <button class="flex-1 py-3 px-4 border-none rounded-lg text-base font-semibold cursor-pointer transition-all bg-surface-alt text-text hover:bg-border" (click)="cancelResume()">{{ 'COMMON.CANCEL' | translate }}</button>
              <button class="flex-1 py-3 px-4 border-none rounded-lg text-base font-semibold cursor-pointer transition-all bg-[image:var(--gradient-primary)] text-text-inverse hover:shadow-[var(--shadow-hover)]" (click)="confirmResume()">{{ 'EXAM.RESUME_BTN' | translate }}</button>
            </div>
          </div>
        </div>
      }
...
export class ExamDetailComponent implements OnInit {
  ...
  protected getSkillKey(skill: SkillType): string {
    switch (skill) {
      case SkillType.Listening:
        return 'COMMON.SKILLS.LISTENING';
      case SkillType.Reading:
        return 'COMMON.SKILLS.READING';
      case SkillType.Writing:
        return 'COMMON.SKILLS.WRITING';
      case SkillType.Speaking:
        return 'COMMON.SKILLS.SPEAKING';
      default:
        return '';
    }
  }

  protected getSkillKeyByName(skill: string): string {
